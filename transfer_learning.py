...
for _ in range(250):
    ...
    dr_input = scipy.ndimage.zoom(X_train[j:j + BATCH_SIZE, :, :, :], zoom=(1, 224 / 32, 224 / 32, 1), order=1)
...
for _ in range(50):
    ...
    dr_input = scipy.ndimage.zoom(X_test[j:j + BATCH_SIZE, :, :, :], zoom=(1, 224 / 32, 224 / 32, 1),
                                  order=1)
    googlenet_out_tensor = sess.run(googlenet_output, feed_dict={googlenet_input: dr_input})
